' Stream: VBA/locbomfull
' File: locbomfull.bas

Attribute VB_Name = "locbomfull"
Sub bom_tc14_full()
Call Focus(True)
...
Set wbth = Workbooks(ThisWorkbook.Name) 'wb tonghop
    ...
    tenmay = uf_mainmenu.tb_tenloaimay
    On Error GoTo loi1
...
loi1:
    ...
    'vong lap trong file plm full
        For i = 3 To dc
            ...
            sttdongitemname = Right(vitriitemname, Len(vitriitemname) - 3)
            End If
            'neu khong ton tai unit trong bo loc thi thoat khoi vong lap plm full
            If vitriitemname = "" Then
            Exit For
            'neu ton tai unit trong bo loc, Unit co the bung, level bom cua dong hien tai =6 thi xoa luon dong do
            ...
            dc = ws_Plm.Range("A" & Rows.Count).End(xlUp).Row
            i = sttdongitemname - 1
            'neu ton tai unit trong bo loc, unit co the bung, level bom cua dong hien tai nho hon level bom cua dong ke tiep thi tim khoang can xoa va xoa di.
            ...
            vitridencungcap = Right(vitrilevelbom, Len(vitrilevelbom) - 3)
                For vttrunggian = sttdongitemname + 1 To vitridencungcap
                    ...
                    End If
                Next
            'neu ton tai unit trong bo loc, Unit co the bung, level bom cua dong hien tai > level bom cua dong ke tiep, va level bom cua dong hien tai <6 thi khong lam gi.
            ...
            'neu ton tai unit trong bo loc, Unit khong the bung, thi xoa du lieu cua dong hien tai.
