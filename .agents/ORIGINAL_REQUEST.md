...
### R4. Giao diện Người dùng (Desktop GUI cho Người Dùng Non-Tech & i18n)
- **Thiết kế thân thiện Non-Tech**: Giao diện tối giản, trực quan, loại bỏ các nút chức năng rườm rà. Quy trình thao tác 1 chạm (One-Click Pipeline) hoặc Wizard 3 bước rõ ràng.
- **Đa ngôn ngữ (i18n)**: Hỗ trợ chuyển đổi tức thì giữa 3 ngôn ngữ: **Tiếng Việt (VN)**, **Tiếng Nhật (JP)**, và **Tiếng Trung (CN)** qua bộ từ điển JSON, không hardcode ngôn ngữ.

### R5. Kiến Trúc Mở Rộng Linh Hoạt (Adapter Pattern)
- **Cô lập Core so sánh**: Lõi so sánh BOM và thuật toán cây phân cấp hoàn toàn tách biệt khỏi tầng thu thập dữ liệu.
- **Interface Provider**:
  - `PLMProvider`: Hỗ trợ cắm rút giữa `TeamcenterSeleniumAdapter` và `ExcelPLMAdapter`.
  - `ERPProvider`: Hỗ trợ cắm rút giữa `SAPR3COMAdapter` và `ExcelR3Adapter`.
  - Khi hệ thống Web PLM hoặc SAP thay đổi giao diện/API trong tương lai, chỉ cần cập nhật Adapter tương ứng mà **tuyệt đối không phá vỡ Core lõi**.

### R6. Chuẩn Mã Nguồn & Cấu Trúc Thư Mục
- Tuân thủ nghiêm ngặt **PEP8**, chú thích rõ ràng, Type Hints đầy đủ.
- **Cấu trúc thư mục phẳng (Flat Production Structure)**: Bố cục gọn gàng, tránh chia nhỏ tầng thư mục quá sâu gây khó khăn cho bảo trì và vận hành.
- **Tuân thủ Speckit**: Tài liệu hóa chi tiết đặc tả hợp đồng dữ liệu (Schemas), ca kiểm thử và quy chuẩn nghiệm thu trước khi code.

### R7. Đóng Gói & Tự Động Cập Nhật Chuẩn HASH_ONLY_LAN (Theo MP2027 Standard)
...
- [x] Chạy thử nghiệm trích xuất dữ liệu đăng nhập từ web http://tcmp3gwb:3000/.
- [x] Xây dựng bộ test so sánh kết quả logic Python với file mẫu form_ssbom.xlsm để đảm bảo kết quả đối soát khớp 100%.
- [x] Kiểm thử chuyển đổi giao diện động giữa 3 ngôn ngữ VN / JP / CN (i18n).
- [x] Kiểm thử tính độc lập của Core Engine khi thay đổi Provider đầu vào (Adapter pattern).
- [x] Đóng gói chương trình theo đúng chuẩn MP2027 (`<App>_Launcher.exe`, Inno Setup installer, bundle `apps/<version>/` và script `package_app.py` sinh `.mpupdate` + `latest.json`).
